...
stopuser = {}
token = '5249489492:AAH7646EV0w2oEtIJ_wkVVxDKP8Vr5pbGHQ'
...
def start(message):
  def my_function():
    ...
    if BL == '𝐅𝐑𝐄𝐄':	
      ...
      contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/DREAM_ASA")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
  ''',reply_markup=keyboard)
      return
    keyboard = types.InlineKeyboardMarkup()
    contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/Deam_Chk_Team")
    keyboard.add(contact_button)
...
𝘽𝙤𝙩 𝘽𝙮 @DREAM_ASA''', reply_markup=mes)

          msg=f'''<b>𝐀𝐏𝐏𝐑𝐎𝐕𝐄𝐃 ✅

𝐂𝐂 -> <code>{cc}</code>
- - - - - - - - - - - - - - - - - - - - - - -
𝐆𝐀𝐓𝐄𝐖𝐀𝐘 -> <code>{gate}</code>
𝐑𝐄𝐒𝐏𝐎𝐍𝐒𝐄 -> <code>{last}</code>
- - - - - - - - - - - - - - - - - - - - - - -
𝐁𝐈𝐍 -> <code>{cc[:6]}</code>
𝐁𝐈𝐍 𝐈𝐍𝐅𝐎 -> <code>{card_type} - {brand}</code>
𝐁𝐀𝐍𝐊 -> <code>{bank}<code>
𝐂𝐎𝗨𝐍𝐑𝐓𝐘 -> <code>{country} - {country_flag}</code>
- - - - - - - - - - - - - - - - - - - - - - -
𝐓𝐈𝐌𝐄 𝐓𝐀𝐊𝐄𝐍 -> {"{:.1f}".format(execution_time)}
𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑 :@DREAM_ASA</b>
𝐂𝐇𝐀𝐍𝐍𝐄𝐋:@Dream_Chk_Team</b>'''
...
def respond_to_vbv(message):
  def my_function():
    ...
    try:
      ...
      msg=f'''<b>𝐃𝐑𝐄𝐀𝐌 𝗩𝗜𝗣 𝗦𝗨𝗕𝗦𝗖𝗥𝗜𝗕𝗘𝗗 ✅
𝑺𝑼𝑩𝑺𝑪𝑹𝑰𝑷𝑻𝑰𝑶𝑵 𝐄𝐗𝐏𝐈𝐑𝐄𝐃 𝐈𝐍 -> {timer}
𝗧𝗬𝗣 -> {typ}</b>'''
...
def start(message):
  def my_function():
    ...
    try:
      ...
      pas='DREAM-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
      current_time = datetime.now()
